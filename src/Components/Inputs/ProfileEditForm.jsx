...
const InputField = ({ label, name, value, onChange, type = "text", disabled = false, required = false }) => (
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    <div className="flex justify-between items-center pb-4">
                        <h2 className="text-xl font-bold text-gray-800">
                            {activeTab === 'profile' ? 'Update Information' : 'Change Password'}
                        </h2>
                    ...
                    {activeTab === 'profile' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="col-span-1 md:col-span-2 flex items-center gap-4 mb-2">
                                ...
                                <div className="flex-1">
                                    ...
                                    <div className="flex gap-2 items-center">
                                        ...
                                        <span className="text-xs text-gray-500">Max 5MB (JPEG, PNG, GIF)</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <select name="gender" value={formData.gender || ""} onChange={handleChange} className="border border-gray-300 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-violet-500">
                            ...
                            <InputField label="Year" name="year" value={formData.year} onChange={handleChange} type="number" />
                            <InputField label="Semester" name="semester" value={formData.semester} onChange={handleChange} type="number" />

                            <div className="col-span-1 md:col-span-2">
                                <label className="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" name="is_hosteler" checked={formData.is_hosteler || false} onChange={handleChange} />
                                    <span className="text-sm font-medium text-gray-700">Is Hosteler</span>
                                </label>
                            </div>

                            {formData.is_hosteler && (
                                <>
                                    <InputField label="Hostel Name" name="hostel_name" value={formData.hostel_name} onChange={handleChange} />
                                    <InputField label="Room Number" name="room_number" value={formData.room_number} onChange={handleChange} />
                                </>
                            )}
                            <div className="col-span-1 md:col-span-2">
                                <InputField label="Designation (Staff Only)" name="designation" value={formData.designation} onChange={handleChange} />
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t">
                        ...
                        <button
                            ...
                            disabled={loading || uploading}
                        >
                            {loading ? "Saving..." : activeTab === 'profile' ? "Save Changes" : "Update Password"}
                        </button>
                    </div>
                </form>
            </div>
