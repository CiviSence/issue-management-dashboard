        ...
        <>
            ...
            <div className="w-full p-4 lg:w-[calc(100vw-15vw)] bg-background text-foreground min-h-screen overflow-y-auto transition-colors duration-200">
                <div className="w-full bg-primary p-6 rounded-3xl text-primary-foreground shadow-lg mb-6">
                    ...
                    <p className="text-primary-foreground/80">Manage and update the status of your assigned issues.</p>
                </div>

                <div className="bg-card rounded-2xl shadow-sm p-6 border border-border">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-lg font-bold text-card-foreground">Task List</h2>
                        <Searchbar />
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {assignedIssues.map((issue) => (
                                <div key={issue.id} className="p-5 border border-border rounded-xl hover:shadow-md transition-shadow bg-card">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <h3 className="font-bold text-card-foreground text-lg mb-1">{issue.title}</h3>
                                            <div className="flex gap-2 items-center">
                                                <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${issue.priority === "high" ? "bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400" :
                                                        issue.priority === "medium" ? "bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400" :
                                                            "bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400"
                                                    }`}>
                                                    {issue.priority}
                                                </span>
                                                <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${issue.status === "pending" ? "bg-muted text-muted-foreground" :
                                    ...
                                    {issue.status === "resolved" && issue.resolution_notes && (
                                        <div className="mt-4 p-3 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-100 dark:border-green-900/30">
                                            <p className="text-xs font-bold text-green-800 dark:text-green-400 mb-1">Resolution Notes:</p>
                                            <p className="text-sm text-green-700 dark:text-green-300 italic">{issue.resolution_notes}</p>
                                        </div>
                                    )}
                                </div>
                            ))}
